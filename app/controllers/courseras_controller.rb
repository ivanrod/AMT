class CourserasController < ApplicationController
    ...
    if Coursera.where(coursera_web_id: params[:coursera_id].to_i) == []
	    ...
	    @course.users.push(current_user)
			...
			if @course.save
				Coursera.create(course_id: @course.id, coursera_web_id: params[:coursera_id])
	    ...
	    @action = "Importar curso"
			...
			flash[:error] = "Este curso ya existe"
